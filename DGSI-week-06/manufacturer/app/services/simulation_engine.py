...
class SimulationEngine:
    ...
    def _process_purchase_orders(self) -> List[Dict]:
        """Process purchase orders due today (internal suppliers only)."""
        events = []
        from app.models.purchase_order import Supplier

        # Get all pending or partial POs from INTERNAL suppliers
        # External suppliers are handled by polling in ExternalSupplierService
        all_pos = self.db.query(PurchaseOrder).join(Supplier).filter(
            PurchaseOrder.status.in_(["pending", "partial"]),
            Supplier.is_external == False
        ).all()
